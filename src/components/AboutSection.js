  ...
  return (
    <div ref={aboutRef} id="about-section" className='about-section md:px-16 px-4 md:py-4 h-85vh w-full bg-gradient-to-b from-indigo-300 to-indigo-100 flex flex-col items-center justify-evenly md:space-y-0'>
      ...
      <div className='introduction-container flex md:flex-row flex-col md:space-x-12 md:space-y-0 space-y-4 md:w-full w-[85%] max-h-72 md:mb-5 md:p-6 p-4 md:mb-6 items-center rounded bg-indigo-100 shadow'>
        ...
        <p className='font-poppins md:text-lg text-sm leading-relaxed md:overflow-hidden overflow-scroll'>Hello, my name is Vincenzo I'm a frontend developer with a passion for making cool-looking web applications. My coding journey began with python and SQL and has since transitioned into a more web development focus of HTML, CSS, and javascript. In the future, I'm looking forward to learning more about the backend world of web development and further increasing my skills as a developer. When I have spare time, I enjoy watching soccer and hockey (go Leafs go!) and playing video games but most importantly learning new things.</p>
      </div>
