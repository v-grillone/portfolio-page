...
const HobbyArrow = ( {direction, changeHobby, clickHobbyArrow, setClickHobbyArrow, iconPosition} ) => {
  ...
  return (
    <svg width="70" height="30" viewBox="0 0 90 50" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${direction === 'left' ? 'rotate-180' : ''}`}>
      ...
      <path id={`${direction === 'left' ? 'left-next-arrow' : 'right-next-arrow'}`} d="M70 25L32.5 46.6506L32.5 3.34937L70 25Z" fill={direction === 'left' & iconPosition.icon5 === 'left-[50%]' ? '#d1d5db' : direction === 'right' & iconPosition.icon1 === 'left-[50%]' ? '#d1d5db' :'#1E40AF'} fillOpacity="0.7"/>
      <path id={`${direction === 'left' ? 'left-next-arrow' : 'right-next-arrow'}`} opacity="0.7" d="M50 25L12.5 46.6506L12.5 3.34937L50 25Z" fill={direction === 'left' & iconPosition.icon5 === 'left-[50%]' ? '#6b7280' : direction === 'right' & iconPosition.icon1 === 'left-[50%]' ? '#6b7280' :"#7DD3FC"} fillOpacity="0.7"/>
      </g>
